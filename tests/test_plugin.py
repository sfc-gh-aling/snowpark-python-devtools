...
def plugin_was_loaded(pytestconfig):
    ...
    assert pytestconfig.pluginmanager.hasplugin("snowpark-python-devtools")
...
def test_basic_marker_run_selective(pytester, live_server):
    ...
    # with @pytest.mark.snowpark_vcr in record and replay mode
    ...
    # with @pytest.mark.snowpark_vcr in record and replay mode
    result = pytester.runpytest("-v", "--snowpark-record-tests-selection=annotated")
    result.assert_outcomes(passed=2)
    cassette_dir_path = pytester.path / "cassettes"
    assert (
        os.listdir(cassette_dir_path) == ["test_with_marker.yaml"]
    ...
    # with @pytest.mark.snowpark_vcr in record and replay mode
    result = pytester.runpytest("-v", "--snowpark-record-tests-selection=all")
    result.assert_outcomes(passed=2)
    cassette_dir_path = pytester.path / "cassettes"
    assert (
...
def test_basic_marker_run_disable(pytester, live_server):
    ...
    # with @pytest.mark.snowpark_vcr in record and replay mode
    result = pytester.runpytest("-v", "--snowpark-record-tests-selection=disable")
    result.assert_outcomes(passed=2)
